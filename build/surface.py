...
def _lattice_2D(formula, a1, a2, angle, size = (1, 1, 1), center = False, vacuum = None):
    """Create a rectangular 2D lattice

    Parameters
    ----------
    formula : string, chemical symbol
    a1 : float, lattice parameter in the (1,0,0) direction
    a2 : float, lattice parameter in the (cos(angle) * a2, sin(angle) * a2, 0) direction
    angle: float, angle in radians between the a1 and a2 lattice vectors
    size : tuple, size of lattice in a1 and a2 direction 3rd should always be 1
    center : boolean, whether to add an atom in the center of the lattice
    vacuum : float, >= 0, how much vacuum to add
    """
    cell = [[a1, 0, 0], [a2 * np.cos(angle), a2 * np.sin(angle), 0], [0, 0, 0]]
    atoms = Atoms(formula, cell = cell, pbc = (1, 1, 0))
    if center:
        #catom = Atoms(formula, positions = [((np.sqrt(a1 ** 2 + a2 ** 2) / 2 ) * np.cos(np.arctan(a2 / a1)), (np.sqrt(a1 ** 2 + a2 ** 2) / 2 ) * np.sin(np.arctan(a2 / a1)), 0)])
        catom = Atoms(formula, positions = [np.array(cell[0]) / 2 + np.array(cell[1]) / 2])
        atoms.extend(catom)
    if vacuum is not None:
        atoms.center(vacuum, axis = 2)
    atoms = atoms.repeat(size)
...
def hexagonal(formula : str, a : float, size = (1, 1, 1), vacuum = None):
    """Create a 2D hexagonal lattice

    Parameters
    ----------
    formula : string, chemical symbol
    a : float, lattice parameter in the (1, 0, 0) direction
    size : tuple, size of lattice in a1 and a2 direction 3rd should always be 1
    vacuum : float, >= 0, how much vacuum to add
    """
...
def square(formula : str, a : float, size = (1, 1, 1), vacuum = None):
    ...
    vacuum : float, >= 0, how much vacuum to add
    """
    return _lattice_2D(formula, a1 = a, a2 = a, angle = np.pi / 2, size = size, center = False, vacuum = vacuum) 

def rectangle(formula : str, a1 : float, a2 : float, size = (1, 1, 1), center = False, vacuum = None):
    """Create a 2D rectangular lattice

    Parameters
    ----------
    formula : string, chemical symbol
    a1 : float, lattice parameter in the (1,0,0) direction
    a2 : float, lattice parameter in the (0,1,0) direction
    size : tuple, size of lattice in a1 and a2 direction 3rd should always be 1
    center : boolean, whether to add an atom in the center of the lattice
    vacuum : float, >= 0, how much vacuum to add
    """
    return _lattice_2D(formula, a1 = a1, a2 = a2, angle = np.pi / 2, size = size, center = center, vacuum = vacuum) 

def oblique(formula : str, a1 : float, a2 : float, angle : float, size = (1, 1, 1), center = False, vacuum = None):
    """Create a 2D rectangular lattice
    ...
    a1 : float, lattice parameter in the (1,0,0) direction
    a2 : float, lattice parameter in the (0,1,0) direction
    angle : float, in radians, angle between a1 (1,0,0) and a2 directions
    size : tuple, size of lattice in a1 and a2 direction 3rd should always be 1
    center : boolean, whether to add an atom in the center of the lattice
    vacuum : float, >= 0, how much vacuum to add
    """
    return _lattice_2D(formula, a1 = a1, a2 = a2, angle = angle, size = size, center = center, vacuum = vacuum) 
